...
function getSelectedOptionKey(
  ...
  const byOptionKey = q.answers.find((a: Answer) => a.optionKey === selectedValue);
...
const WorkingWithShapesScore: React.FC<WorkingWithShapesScoreProps> = ({
  ...
  const workingWithShapesQs = useMemo(() => {
    return questions.filter((q: QuestionData) => q.primaryTrait === "Working with Shapes");
  }, [questions]);

  const totalQuestions = workingWithShapesQs.length || 3;
 ...
 <div className="w-full max-w-2xl mx-auto px-2 py-0">
    ...
    <div className="flex items-center justify-between">
      <h1 className="mb-1 text-[12px] font-semibold text-gray-700 leading-none"> Working with Shapes</h1>
