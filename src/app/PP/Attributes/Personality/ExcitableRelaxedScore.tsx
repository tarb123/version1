...
type Props = {
  ...
  title?: string;
  leftLabel?: string;  // Excitable
  rightLabel?: string; // Relaxed
};

/** Accepts: "A"/"B", 1/2, "Exc_Rel_1A", "Exc_Rel_2B" */
...
export default function ExcitableRelaxedScore({
  questions,
  responses,
  traitKey = "Excitable vs Relaxed",
  title = "Excitable vs Relaxed",
  leftLabel = "Excitable",
  rightLabel = "Relaxed",
}: Props) {
  const { scaledScore, strength, region } = useMemo(() => {
    ...
    for (const q of forcedQs) {
      ...
      // default meaning: A = Excitable (left), B = Relaxed (right)
      const reverse =
        typeof (q as unknown as { reverse?: boolean }).reverse === "boolean"
          ? Boolean((q as unknown as { reverse?: boolean }).reverse)
          : false;

      const isRelaxed = reverse ? pick === "A" : pick === "B";
      if (isRelaxed) relaxedCount += 1;
    }
    ...
    const ratio = relaxedCount / answeredCount; // 0..1
  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-8 py-0">
      ...
      <div className="mt-2 relative">
        {/* Side labels */}
        <span className="absolute -left-2 top-2 -translate-y-1/2 text-[10px] font-bold text-gray-500">
          {leftLabel}
        </span>
        <span className="absolute right-0 top-2 -translate-y-1/2 text-[10px] font-bold text-gray-500">
          {rightLabel}
        </span>
