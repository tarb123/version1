...
type Props = {
  ...
  traitKey?: string;
  title?: string;
  leftLabel?: string;  // Frank
  rightLabel?: string; // Social Desirability
};

/** Accepts: "A"/"B", 1/2, "Frank_SD_1A", "Frank_SD_2B" */
...
export default function FrankSocialDesirabilityScore({
  questions,
  responses,
  traitKey = "Frank vs Social Desirability",
  title = "Frank vs Social Desirability",
  leftLabel = "Frank",
  rightLabel = "Social Desirability",
}: Props) {
  const { scaledScore, strength, region } = useMemo(() => {
    ...
    for (const q of forcedQs) {
      ...
      const isSocialDes = reverse ? pick === "A" : pick === "B";
      if (isSocialDes) bCount += 1;
    }
    ...
    const ratio = bCount / answeredCount; // 0..1
  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-8 py-0">
      ...
      <div className="mt-2 relative">
        ...
        <span className="absolute -right-12 top-2 -translate-y-1/2 text-[10px] font-bold text-gray-500">
          {rightLabel}
        </span>
