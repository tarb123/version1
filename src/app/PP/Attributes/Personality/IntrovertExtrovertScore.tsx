...
type Props = {
  ...
  leftLabel?: string;  // Introvert
  rightLabel?: string; // Extrovert
};

/** Accepts: "A"/"B", 1/2, "IE_1A", "IE_2B" */
...
export default function IntrovertExtrovertScore({
  questions,
  responses,
  traitKey = "Introvert vs Extrovert",
  leftLabel = "Introvert",
  ...
  const { scaledScore, strength, region } = useMemo(() => {
    ...
    for (const q of forcedQs) {
      ...
      // Default meaning: A=Introvert, B=Extrovert
      const reverse =
        typeof (q as unknown as { reverse?: boolean }).reverse === "boolean"
          ? Boolean((q as unknown as { reverse?: boolean }).reverse)
          : false;

      const isExtrovert = reverse ? pick === "A" : pick === "B";
      if (isExtrovert) extrovertCount += 1;
    }
    ...
    const ratio = extrovertCount / answeredCount; // 0..1
  ...
  const indicatorLeftPct =
    scaledScore > 0 ? ((scaledScore - 0.5) / 7) * 100 : 0;
