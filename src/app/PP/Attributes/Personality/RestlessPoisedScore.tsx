...
type Props = {
  ...
  title?: string;
  leftLabel?: string;  // Restless
  rightLabel?: string; // Poised
};

/** Accepts: "A"/"B", 1/2, "Rest_Poi_1A", "Rest_Poi_2B" */
...
export default function RestlessPoisedScore({
  questions,
  responses,
  traitKey = "Restless vs Poised",
  title = "Restless vs Poised",
  leftLabel = "Restless",
  rightLabel = "Poised",
}: Props) {
  const { scaledScore, strength, region } = useMemo(() => {
    ...
    let answeredCount = 0;
    let poisedCount = 0; // B = Poised by default
    ...
    for (const q of forcedQs) {
      ...
      // Optional reverse support if you ever add reverse:true in questions
      const reverse =
        typeof (q as unknown as { reverse?: boolean }).reverse === "boolean"
          ? Boolean((q as unknown as { reverse?: boolean }).reverse)
          : false;

      // Default: A = Restless, B = Poised
      const isPoised = reverse ? pick === "A" : pick === "B";
      if (isPoised) poisedCount += 1;
    }
    ...
    const ratio = poisedCount / answeredCount; // 0..1
  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-8 py-0">
      ...
      <div className="mt-2 relative">
        ...
        <span className="absolute right-1 top-2 -translate-y-1/2 text-[10px] font-bold text-gray-500">
          {rightLabel}
        </span>
