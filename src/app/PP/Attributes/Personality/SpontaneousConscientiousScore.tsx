...
/** Accepts: "A"/"B", 1/2, "Sub_Asrt_1A", "Sub_Asrt_2B" */
...
export default function SpontaneousConscientiousScore({
  questions,
  responses,
  traitKey = "Spontaneous vs Conscientious",
  leftLabel = "Spontaneous",
  rightLabel = "Conscientious",
}: Props) {
  const { scaledScore, strength, region } = useMemo(() => {
    ...
    for (const q of forcedQs) {
      ...
      const isAssertive = reverse ? pick === "A" : pick === "B";
      if (isAssertive) assertiveCount += 1;
    }
    ...
    const ratio = assertiveCount / answeredCount; // 0..1
 ...
 return (
    <div className="w-full max-w-4xl mx-auto px-8 py-0">
      ...
      <div className="mt-2 relative">
        ...
        <span className="absolute -left-7 top-2 -translate-y-1/2 text-[10px] font-bold text-gray-500">
          {leftLabel}
        </span>
        <span className="absolute -right-8 top-2 -translate-y-1/2 text-[10px] font-bold text-gray-500">
          {rightLabel}
        </span>
