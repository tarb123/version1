...
type Props = {
  ...
  responses: Responses;
  traitKey?: string;   // must match primaryTrait
  leftLabel?: string;  // Reactive
  rightLabel?: string; // Organized
};

/** Accepts: "A"/"B", 1/2, "Rea_Org_1A", "Rea_Org_2B" */
...
export default function ReactiveOrganizedScore({
  questions,
  responses,
  traitKey = "Reactive vs Organized",
  leftLabel = "Reactive",
  ...
  const { scaledScore, strength, region } = useMemo(() => {
    ...
    let answeredCount = 0;
    let organizedCount = 0; // ✅ B = Organized
    ...
    for (const q of forcedQs) {
      ...
      // Optional support if you add reverse later
      const reverse =
        typeof (q as unknown as { reverse?: boolean }).reverse === "boolean"
          ? Boolean((q as unknown as { reverse?: boolean }).reverse)
          : false;

      const isOrganized = reverse ? pick === "A" : pick === "B";
      if (isOrganized) organizedCount += 1;
    }
    ...
    const ratio = organizedCount / answeredCount; // 0..1
  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-8 py-0">
      <div className="mt-2 relative">
        {/* Side labels */}
        <span className="absolute -left-1 top-2 -translate-y-1/2 text-[10px] font-bold text-gray-500">
          {leftLabel}
        </span>
        <span className="absolute -right-3 top-2 -translate-y-1/2 text-[10px] font-bold text-gray-500">
          {rightLabel}
        </span>

        {/* shared width container for circle + segments */}
        <div className="px-10">
