...
type Props = {
  ...
  leftLabel?: string;  // Self-Sufficient
  rightLabel?: string; // Group-Oriented
};

/** Accepts: "A"/"B", 1/2, "SS_GO_1A", "SS_GO_2B" */
...
export default function SelfSufficientGroupOrientedScore({
  questions,
  responses,
  traitKey = "Self-Sufficient vs Group-Oriented",
  leftLabel = "Self-Sufficient",
  rightLabel = "Group-Oriented",
}: Props) {
  const { scaledScore, strength, region } = useMemo(() => {
    ...
    let answeredCount = 0;
    let groupCount = 0; // B = Group-Oriented
    ...
    for (const q of forcedQs) {
      ...
      // default: A=Self-Sufficient, B=Group-Oriented
      const isGroup = reverse ? pick === "A" : pick === "B";
    ...
    const ratio = groupCount / answeredCount; // 0..1
  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-8 py-0">
      <div className="mt-2 relative">
        ...
        <span className="absolute -left-8 top-2 -translate-y-1/2 text-[10px] font-bold text-gray-500">
          {leftLabel}
        </span>
        <span className="absolute -right-10 top-2 -translate-y-1/2 text-[10px] font-bold text-gray-500">
          {rightLabel}
        </span>

        {/* Shared width wrapper for circle + segments */}
        <div className="px-10">
